...
from app.schemas.subcon import (
    SubconCreate,
    SubconDetailOut,
    SubconOut,
    SubconProjectAssignRequest,
    SubconProjectOut,
)
...
@router.get("/{subcon_id}", response_model=SubconDetailOut)
...
@router.delete("/{subcon_id}/projects/{project_id}", response_model=SubconDetailOut)
def remove_subcon_project(subcon_id: int, project_id: int, user: UserContext = Depends(permission_required("subproject", "write")), db: Session = Depends(get_db)):
    return subcon_service.remove_subcon_project(db, user, subcon_id, project_id)
