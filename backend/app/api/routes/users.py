...
@router.get("", response_model=list[UserOut], dependencies=[Depends(permission_required("user", "read"))])
def list_users(db: Session = Depends(get_db)):
    return user_service.list_users(db)


@router.get("/{user_id}", response_model=UserOut, dependencies=[Depends(permission_required("user", "read"))])
def get_user(user_id: int, db: Session = Depends(get_db)):
    return user_service.get_user(db, user_id)


@router.post("", response_model=UserOut, dependencies=[Depends(permission_required("user", "write"))])
def create_user(payload: UserCreate, db: Session = Depends(get_db)):
    return user_service.create_user(db, payload)


@router.patch("/{user_id}", response_model=UserOut, dependencies=[Depends(permission_required("user", "write"))])
...
@router.patch("/{user_id}/password", status_code=204, dependencies=[Depends(permission_required("user", "write"))])
