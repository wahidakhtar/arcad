...
export const READ_ONLY_FIELDS = new Set(["budget", "cost", "paid", "balance", "active_fe", "active_provider", "po_number", "invoice_number", "active_po_number", "active_invoice_number", "next_invoice_date", "last_recharge_date", "next_recharge_date"])
...
export function txStatusLabel(typeKey: string | undefined, statusKey: string | undefined, statusLabel: string): string {
  if (statusKey === "exct") {
    if (typeKey === "b_sur" || typeKey === "e_sur") return "Approved"
    if (typeKey === "ref") return "Received"
  }
  return statusLabel
}
